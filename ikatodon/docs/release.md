...
- **新規アップロード**: HEIC / HEIF 画像（iPhone の既定形式）の添付が失敗するようになります。
  `MediaAttachment::IMAGE_MIME_TYPES` には `image/heic` / `image/heif` が残っているため、
  受理はされるが変換段階で落ちる形です
- **既存メディア**: 影響ありません。HEIF はアップロード時に JPEG 等へ変換されて保存されており
